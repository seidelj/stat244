...
Which rejects the null hypothesis that $X \sim Bin(n=5, \theta)$\marginnote{However, it's important to remember that small P-values provide less evidence against the Null.}.
...
Which computes to $\chi^2 = 11.16$.  We check this value with the table, given $(r-1)(c-1)=2\cdot 2=4$ degress of freedom.  From the Chi-Squared table, we'd have a p-value $<.025$ which \marginnote{Small p-values provide strong evidence against Null when using a chi-square test.  P-values are the chance of false positives; when P-values are small the null hypothesis is unlikely.}provides fairly strong evidence against the null hypothesis that the patterns are independent.
