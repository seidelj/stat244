...
\newthought{Evaluating} $\Lambda$ for a few values of $X$ yields.
\begin{table}
\centering
\caption{GLR}
\label{my-label1}
...
\newthought{First}, \marginnote{I've found this in previous homeworks.} recall that the mle of $L(\theta)$ is $\hat{\theta}=\overline{X}$.  Set up the likelihood ratio test
...
Using the table we can compute the mle, $\hat{\theta} = 339/215(5) = .31$.
